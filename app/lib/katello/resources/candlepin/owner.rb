      ...
      class Owner < CandlepinResource
        ...
        class << self
          ...
          def import(organization_name, path_to_file, options)
            ...
            oauth_header = build_oauth_header(self.site + path, :post)
            headers = self.default_headers.except('content-type').merge('Authorization' => oauth_header)
            response = conn.post(path, payload, headers)
            JSON.parse(response.body)
          end
